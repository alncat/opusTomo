...
def run_batch(model, lattice, y, yt, rot, tilt=None, ind=None, ctf_params=None,
    ...
    if not vanilla:
        ...
        z_global = hvd.allgather(z_local)
        ind_global = hvd.allgather(ind)
    ...
    if use_ctf:
        if vanilla:
            ...
            if plot and hvd.rank() == 0:
                ...
                correlation_m = correlations.mean().detach()
            else:
                correlations = F.cosine_similarity(y_recon[:,d_k,...].reshape(B,-1), y_ref[:,d_k,...].reshape(B,-1))
                correlation_m = correlations.mean().detach()
    ...
    return z_mu, z_logstd, z, y_recon, y_recon_tilt, losses, y_ref, mus, euler_samples, y_recon_ori, neg_mus, mask_sum, decout["affine"], correlation_m
...
def main(args):
    ...
    hvd.init()
    log("hvd rank: {}".format(hvd.local_rank()))
    ...
    log(f'# of training samples is {len(data_generator)*args.batch_size}, {hvd.rank()}')

    #load symmetry operators
    if args.symm:
        symm_pkl = torch.load(args.symm)
        log(f"loaded {args.symm}")
        #convert quats to rotation matrix
        identity_quats = torch.tensor([[1, 0, 0, 0]])
        symm_ops = torch.cat([identity_quats, symm_pkl["symm_ops"][:2]], dim=0)
        symm_ops = lie_tools.quaternions_to_SO3_wiki(symm_ops)
        #retrieve weights
        symm_ops_weights = symm_pkl["weights"][:2]
        #symm_ops_weights = torch.cat([1 - symm_ops_weights.sum().unsqueeze(-1), symm_ops_weights], dim=0)
        symm_ops_weights = torch.tensor([0.5, 0.25, 0.25])
        print(symm_ops, symm_ops_weights)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        #for minibatch in data_generator:
            ...
            B = len(ind)
            batch_it += B
            global_it = Nimg_train*epoch + batch_it*hvd.size()
            ...
            z_mu, loss, gen_loss, snr, l1_loss, tv_loss, mu2, std2, mmd, c_mmd, mse, body_poses_pred, c_m = \
            ...
            if batch_it % (args.log_interval*10) == 0 and hvd.rank() == 0:
        ...
        #print("total correlation: {}".format(c_m_avg/))
        flog('# =====> Epoch: {} Average training gen_loss = {:.6}, SNR2 = {:.6f}, '\
             'total loss = {:.6f}, avg correlation = {:.6f}; Finished in {}'.format(epoch+1,
                                                         gen_loss_accum/Nimg_train*hvd.size(),
                                                         snr_accum/Nimg_train*hvd.size(), loss_accum/Nimg_train*hvd.size(),
                                                         c_m_avg/batch_it*B, dt.now()-t2))
        ...
        if args.checkpoint and epoch % args.checkpoint == 0 and hvd.rank() == 0:
