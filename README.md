...
| --checkpoint | write ```weights.N.pkl``` and ```z.N.pkl``` every this many epochs, default is 1 |
| --seed | random seed; without it one is drawn and recorded in the ```Namespace(...)``` line of ```run.log```, so a run can be repeated exactly by passing that value back |
| --num-workers | dataloader workers per rank, default is 16. The total is this times ```--nproc_per_node```, so lower it if you run out of host memory |
| --max-threads | upper bound on CPU cores used for FFT parallelization, default is 16 |
| --log-interval | logging interval counted in subtomograms, default is 1000 |
| --window-r | radius of the real-space window applied to the input subtomogram, default is 0.85. ```--no-window``` turns the window off |
| --norm | data normalization given as shift and 1/scale; by default the shift is 0 and the scale is estimated from the dataset |
| --uninvert-data | keep the data sign as it is; by default the sign is inverted |
| --wd | weight decay of the adam optimizer, default is 0 |
| --readctf | meant to read the 3D CTF volume of each subtomogram instead of rebuilding it from the per-tilt parameters. ***Currently not usable***: ```get_warp3dctfs``` returns ```None``` in place of the volume list, so a batch fails with ```TypeError: 'NoneType' object is not subscriptable``` |
| --normalizectf | normalize the CTF by the backprojected grid weights |
| --do-pose-sgd | refine the poses by gradient descent, in addition to the per-subtomogram correction of ```--estpose```. ```--pretrain``` sets how many epochs run with fixed poses first (default 1), ```--pose-lr``` the pose learning rate (default 3.e-4) |
| --optimize-b | optimize the b factor during training |
| --ind | a pkl of particle indices. ***Not honoured for WARP subtomograms***: ```LazyTomoWARPMRCData``` accepts the argument and never applies it, so the whole star is trained on. Slice the star with ```dsd filter_star``` instead |
| --amp | mixed precision; ```train_tomo_dist``` does not support it |

### Warp CTF cache <div id="ctfcache">

With ```--warp```, OPUS-ET reads one CSV of per-tilt CTF parameters for every subtomogram at
startup and turns them into an ```(N, len_tilt, 9)``` array. For 17215 subtomograms that took
about two minutes, and every rank of a distributed run paid it again. The parsed array is now
cached, so only the first run pays.

Left alone, three files appear next to the input starfile, named with a hash of the tilt settings:

```
ribo.star.warp_ctf_<tag>.npy       the parsed parameters
ribo.star.warp_ctf_<tag>.npy.json  shape, dtype and the key the cache was built for
ribo.star.warp_ctf_<tag>.npy.lock  an empty file used to serialize the first build
```

One rank builds the cache while the others wait on the lock, then all of them map it
copy-on-write, so the ranks share one copy through the page cache. On the 17215-subtomogram
ribosome set the first build takes about 21 s and later starts about 1 s, against 123 s for the
old parser.

| argument | explanation |
| --- | --- |
| --ctf-cache | where to keep the cache. Omit it for the sidecar above; give a path to put the cache elsewhere; give ```none``` to parse the CSVs on every start and write nothing |
| --ctf-cache-workers | threads used only while the cache is first built, default is 16. Lower it if the filesystem is under load |

The default needs a writable starfile directory. A read-only one fails with
```RuntimeError: [Errno 13] Permission denied: '....npy.lock'```, which either
```--ctf-cache none``` or ```--ctf-cache /some/writable/path.npy``` avoids.

The cache is keyed on the list of CSV paths together with ```--tilt-step```, ```--tilt-range```
and ```--tilt-limit```. A different starfile or a damaged sidecar is rebuilt automatically, and
different tilt settings write a separate file rather than replacing the first. ***The key does
not cover the contents of the CSV files***: after re-running the CTF estimation in WARP over the
same paths, delete ```<starfile>.warp_ctf_*.npy*``` so the parameters are read again.
...
Besides the analysis result, ```--kpc``` writes the selected particles to
```analyze.filter.16/ind.filter.16.pkl``` and, when the starfile recorded in ```config.pkl``` is
still in place, the matching ```analyze.filter.16/particles.filter.16.star```. The tomogram
datasets index the starfile row for row, so those indices are starfile rows and the star can be
fed straight back into training. If the starfile has moved, run
```dsd filter_star <starfile> analyze.filter.16/ind.filter.16.pkl -o particles.filter.16.star```
...
```
to split the ```consensus_data.star``` into different subsets for different clusters. This is a
convenience wrapper: the split itself is done by ```dsd filter_star```, which is also available
on its own and is the command to reach for when no pose file is being written.

```
dsd filter_star /work/consensus_data.star analyze.filter.19/ind.filter.19.pkl -o subset.star
dsd filter_star /work/consensus_data.star --labels analyze.19/kmeans20/labels.pkl --outdir analyze.19/kmeans20/
dsd filter_star /work/consensus_data.star analyze.filter.19/ind.filter.19.pkl --labels analyze.filter.19/kmeans30/labels.pkl --outdir analyze.filter.19/kmeans30/
```

The first form keeps the rows named by an index file, the second writes one starfile per cluster.
The third is the ```--kpc``` case: the index file restricts the split to the particles that were
re-clustered, which leaves out the ones carrying the sentinel label. Every form keeps
```data_optics``` and any other block of a RELION 3.1 file, which the writer used by
```parse_pose_star``` drops.

| argument | explanation |
| --- | --- |
| star | the starfile to slice, i.e. the one that was used for training |
| ind | index file selecting the rows to keep, either the ```.pkl``` written by ```analyze --kpc``` or a ```.txt``` of integers. Optional, and with ```--labels``` it restricts the split to those rows |
| -o | output starfile, required unless ```--labels``` is given |
| --labels | cluster labels from ```kmeans{K}/labels.pkl```, one entry per row of the input starfile |
| --outdir | directory for the per-cluster starfiles written by ```--labels``` |
| --prefix | name prefix for those files, so label i goes to ```{prefix}{i}.star```, default is ```pre``` |
| --skip-label | comma separated labels to leave out. ```analyze --kpc``` labels the particles it did not select with a value equal to its cluster count, so ```--skip-label 20``` drops them from a ```kmeans20``` split |
| --block | name of the data block to slice; by default the first block that looks like a particle block |

```analyze``` writes ```labels.pkl``` in the ***original*** stack numbering, one entry per row of
