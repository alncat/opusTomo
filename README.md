...
OPUS-TOMO make positive x axis rotate to negative z axis under negative tilt angle, which can be expressed as $`x' = R(\theta_Imod)x`$ (the tilt angle is inverted once again in ```ctf.py```, so it revert to the original tilt angle in IMOD's convention). However, the defocus gradient, i.e., which part of x axis has higher underfocus with negative tilt angle, might be trickier! (You can check whether the negtive x axis or positive x axis swings to higher underfocus under negative tilt angle by estimating the defoci of left or right part. If the negative x axis swings to higher underfocus, then positive z axis has larger underfocus, vice versa. I am preparing a script for checking this!) 
