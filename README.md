...
Firstly, we need to create an aligned stack in the micrograph directory using ```newstack -pa newst.com```.
The ctf estimation can be performed by ```ctfplotter -pa ctfplotter.com```.
...
to the starfile generated by ```relion_ctf_prepare.py```. The operations on starfile can be conveniently handled by starpy at https://github.com/fuzikt/starpy.
