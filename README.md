...
which can be created by newstack command in IMOD with a 'newst.com' configuration file (this is generated by aretomo automatically after reconstruction),
...
The starfile then contains the orientation of each subtomogram, which allows further filtering using OPUS-TOMO.
...
to the starfile generated by ```relion_ctf_prepare.py```.
